...
Along the way I opened eight small fixes for things I found, before asking for the project ([#1405 through #1419](https://github.com/owtf/owtf/pulls?q=is%3Apr+author%3Asaurabh4269)): a deprecated `logging.warn()` call still in use instead of `logging.warning()`, a UTF-8 decode that silently kills plugins on binary tool output, and a SIGKILL escalation loop that was dead code because of an inverted condition. Each one meant reading a module closely enough to understand it.
